...
interface QuestionProps {
  title: string
  children: ReactNode
  /** Abre la primera de entrada, para que se vea que el resto se despliega. */
  defaultOpen?: boolean
}

/**
 * Acordeon con <details> nativo: sin estado, sin JavaScript y sin ARIA.
 * El elemento ya trae abrir y cerrar con Enter o Espacio y el anuncio de
 * "contraido / expandido" en los lectores de pantalla; anadir role o
 * aria-expanded a mano solo puede empeorarlo.
 *
 * El <h2> dentro del <summary> es valido: su modelo de contenido admite
 * contenido de encabezado, y asi el indice del documento no se pierde.
 */
// list-none quita el triangulo en Chrome y Firefox; Safari lo dibuja con un
// pseudo-elemento propio y necesita la segunda regla.
const SUMMARY =
  'flex cursor-pointer list-none items-center justify-between gap-4 py-3 [&::-webkit-details-marker]:hidden'

function Question({ title, children, defaultOpen = false }: QuestionProps) {
  return (
    <details open={defaultOpen} className="accordion group border-b border-current/25">
      ...
      <div className="space-y-2 pb-4 text-sm">{children}</div>
    </details>
  )
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border-t border-current/25">
        <Question title="¿Qué son las naturalezas?" defaultOpen>
          <p>
            Cada Pokémon tiene una de las 25 naturalezas. Una naturaleza sube un 10 % una
            estadística y baja un 10 % otra, entre las cinco que no son PS: ataque, defensa, ataque
            especial, defensa especial y velocidad. Los PS nunca se ven afectados, porque su fórmula
            no incluye el multiplicador de naturaleza.
          </p>
          <p>
        ...
        <Question title="¿Cómo se elige la naturaleza recomendada?">
          <p>
            ...
            daño, o su mejor defensa cuando el perfil es defensivo. Se baja siempre la estadística
            que no usa, normalmente el ataque que no aprovecha.
          </p>
        ...
        <Question title="¿Por qué el x4 solo aparece en Pokémon de doble tipo?">
          <p>
            ...
            multiplican: x2 por uno y x2 por el otro dan x4. Con un solo tipo el máximo posible es
            x2, y el mínimo distinto de cero, x0,5.
          </p>
          <p>
...
Resto = floor((floor((2*Base + IV + floor(EV/4)) * Nivel / 100) + 5) * Naturaleza)`}
          ...
          <p>
            Los <strong>IV</strong> son valores individuales de 0 a 31 con los que nace cada
            Pokémon. Los <strong>EV</strong> son puntos de esfuerzo que se ganan combatiendo, hasta
            252 por estadística. El mínimo que muestra la ficha usa IV 0, EV 0 y naturaleza
            perjudicial; el máximo, IV 31, EV 252 y naturaleza beneficiosa; y el valor neutro, IV 31
            con EV 0 y naturaleza neutra, como referencia realista.
          </p>
        </Question>
        ...
        <Question title="¿De dónde salen los datos?">
          ...
          <p>
            Por eso la aplicación no hace ninguna petición a PokeAPI mientras la usas: todo el
            cálculo ocurre en tu navegador sobre datos ya descargados. Las imágenes sí son remotas,
            servidas desde el repositorio de sprites de PokeAPI.
          </p>
        </Question>
      </div>
    </div>
